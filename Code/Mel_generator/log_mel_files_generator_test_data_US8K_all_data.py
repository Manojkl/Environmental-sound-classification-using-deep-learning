...
def get_embeddings_of_all(files, data_type):
    ...
    total_ommitt = 0
    seg_num = 80
    testing = np.zeros((3147*seg_num,96,64,1))
    # testing = np.zeros((2851*3,96,64,1))
    df_test = pd.read_csv("/home/mkolpe2s/rand/CNN/VGGish/vggish_keras/output.csv")
    ...
    for i, file_name in enumerate(train_file_names):
        ...
        if len(y)>21168:
            for j in range(len(y)//21167):
                ...
                class_value = df_test.loc[df_test['ID'] == int(class_value), 'Prediction'].iloc[0]
    ...
    np.save(str("/home/mkolpe2s/rand/CNN/VGGish/vggish_keras/")+str(data_type)+"_class_value_all.npy",labels)
    np.save(str("/home/mkolpe2s/rand/CNN/VGGish/vggish_keras/")+str(data_type)+"_log_mel_value_all.npy",testing)
