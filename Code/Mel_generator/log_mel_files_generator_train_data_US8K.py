...
def get_embeddings_of_all(files, data_type):
    ...
    total_ommitt = 0
    seg_num = 80
    testing = np.zeros((4642*seg_num,96,64,1))
    ...
    np.save(str("/scratch/mkolpe2s/")+str(data_type)+"_class_value.npy",labels)
    np.save(str("/scratch/mkolpe2s/")+str(data_type)+"log_mel_value.npy",testing)
