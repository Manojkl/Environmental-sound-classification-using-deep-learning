...
import wandb
wandb.init(project="vgg_training_01_dcase_1_frozen_layer")
...
def load_data(ids, x_data, y_data):
    ...
    y_array = []
    labels = ["other", "absence","social_activity","cooking","dishwashing","eating", "vacuum_cleaner", "watching_tv", "working"]
...
X_all = np.load("/scratch/mkolpe2s/Dcase_dev_30_log_mel_value.npy")
Y_all = np.load("/scratch/mkolpe2s/Dcase_dev_30_class_value.npy")
...
set_trainable = False
for layer in vgg.layers:
    if layer.name in ['conv4/conv4_1', 'conv4/conv4_2', 'conv3/conv3_2', 'conv3/conv3_1', 'conv2']:
        set_trainable = True
    if set_trainable:
...
X_all_train = X_all[0:2145759,:,:,:]
Y_all_train = Y_all[0:2145759]
X_all_validate = X_all[2145760:2165759,:,:,:]
Y_all_validate = Y_all[2145760:2165759]
# X_all_test = X_all[2165760:,:,:,:]
# Y_all_test = Y_all[2165760:]
...
train_generator = batch_generator(ids_all_train, 32, x_data=X_all_train, y_data=Y_all_train)
valid_generator = batch_generator(ids_all_validate, 32, x_data=X_all_validate, y_data=Y_all_validate)

STEP_SIZE_TRAIN = len(X_all)//32
STEP_SIZE_VALID = len(X_all)//32
...
model.save(os.path.join("/scratch/mkolpe2s/CNN_dcase/", "vgg_training_08.h5"))